    ...
    # STEP 3: 結果
        ...
        if not st.session_state.dynamic_result:
            with st.spinner("能力を紡ぎ出しています..."):
                analysis_prompt = f"会話履歴から強みを分析しJSONで出力せよ: {{'skills':[], 'jobs':[], 'desc':''}}. 内容: {st.session_state.chat_history}"
        ...
        with col1:
            st.markdown(f"""<div class="tarot-card-outer"><div class="tarot-card-inner">
                ...
                <div style='font-style:italic; margin-top:10px;'>“{dynamic_data.get('desc','運命は開かれた')}”</div>
