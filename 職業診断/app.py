...
def main():
    ...
    # STEP 1: クイズ
        ...
        with col_m2:
            ...
            with st.form("questions_form"):
                for q in QUESTIONS:
                    st.markdown(f"<h3 style='color:#FFD700; text-shadow: 2px 2px 4px #000; font-size:1.4em;'>{q['q']}</h3>", unsafe_allow_html=True)
                    st.radio("選択肢", list(q['options'].keys()), key=q['id'], label_visibility="collapsed", index=None)
                if st.form_submit_button("🔮 真実を明らかにする"):
                    all_answered = True
                    temp_answers = {}
                    for q in QUESTIONS:
                        if st.session_state.get(q['id']) is None:
                            all_answered = False
                            break
                        temp_answers[q['id']] = st.session_state[q['id']]
    ...
    # STEP 3: 結果表示
        ...
        <div class="advice-box">
            <div style="font-weight:900; color:#8B4513; font-size:1.2em; border-bottom:1px solid #8B4513; padding-bottom:5px; margin-bottom:10px;">📜 Oracle's Message</div>
